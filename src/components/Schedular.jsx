...
const Schedular = (props) => {
  ...
  const [eventEnvironments, setEventEnvironments] = useState(
    filterData.environments
  );
  ...
  useEffect(() => {
    ...
    scheduler.templates.hour_scale = function (date) {
      if (date.getMinutes() === 0) {
        var labels = timeZones.map(function (timeZone) {
          ...
          return `<div class="timezone-wrapper">
                        <div class="timezone-label">${timeZoneLabel}</div>
                        <div class="timezone-time">${scheduler.date.date_to_str(
                          "%H:%i"
                        )(timeZoneDate)}</div>
                    </div>`;
        });
    ...
    const filter_inputs = document
      .getElementById("filters_wrapper")
    ...
    scheduler.filter_month =
      scheduler.filter_day =
      scheduler.filter_week =
        function (id, event) {
          // display event only if its type is set to true in filters obj
          // or it was not defined yet - for newly created event
          if (
            filters[event.type] ||
            event.type == scheduler.undefined ||
            filters[event.environment]
          ) {
            return true;
          }
    ...
    scheduler.event(
      document.querySelector("#appointment_form [id='close']"),
      "click",
      close_form
    );
    ...
    scheduler.parse([
      {
        start_date: "2023-05-30 12:00",
        end_date: "2023-05-30 20:00",
        text: "Appointment A-48865",
        type: "appointment",
        environment: "all",
        timezone: "EST",
      },
      {
        start_date: "2023-05-22 14:00",
        end_date: "2023-05-22 16:00",
        text: "Task T-44864",
        type: "task",
        environment: "all",
        timezone: "EST",
      ...
      {
        start_date: "2023-05-26 16:30",
        end_date: "2023-05-26 18:00",
        text: "Training T-46558",
        type: "training",
        environment: "test",
        timezone: "EST",
      },
      {
        start_date: "2023-05-24 18:30",
        end_date: "2023-05-24 20:00",
        text: "Appointment A-45564",
        type: "appointment",
        environment: "all",
        timezone: "EST",
      },
              ...
              return (
                ...
                >
                  <p>
                    ...
                    <i className="material-icons icon_color">add</i>
                  </p>
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          defaultChecked
                        />
                        <i className="material-icons icon_color">check_box</i>
                        {item.label}
                      </label>
                    );
                  })}
